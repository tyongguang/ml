...
void prob(double* x, double* y, int size_x, int size_v) {
    set_value(0.0, y, size_v);
    for(int i = 0; i < size_x; ++i)
        y[(int)x[i]]++;
    for(int i = 0; i < size_v; ++i)
        y[i] /= size_x;
}
void prob(double* x, int* pos, double* y, int size_x, int size_v) {
    set_value(0.0, y, size_v);
    for(int i = 0; i < size_x; ++i)
        y[(int)x[pos[i]]]++;
    for(int i = 0; i < size_v; ++i)
...
double entropy(double* x, int size_x, int size_v) {
    int* v;
    double result = 0.0;
    NEW_POINTERV(v, int, size_v);
    set_value(0, v, size_v);
    for(int i = 0; i < size_x; ++i)
        v[(int)x[i]]++;
    for(int i = 0; i < size_v; ++i) {
        double p = (double)v[i] / (double)size_x;
        if(0.0 == p)
            p = 0.0001;
...
double entropy(double* x, int* pos, int size_x, int size_v) {
    int* v;
    double result = 0.0;
    NEW_POINTERV(v, int, size_v);
    set_value(0, v, size_v);
...
double info(double* x, double* y, int size_x, int size_v) {
    int* v;
    int* v_c;
    double* t;
    double result = 0.0;
    NEW_POINTERV(v, int, size_v);
    NEW_POINTERV(v_c, int, size_v);
    NEW_POINTERV(t, double, size_x);
    set_value(0, v, size_v);
    set_value(0, v_c, size_v);
    set_value(0.0, t, size_x);
    for(int i = 0; i < size_x; ++i) {
        v[(int)x[i]]++;
        v_c[(int)x[i]]++;
    }
    ...
    for(int i = 0; i < size_v; ++i) {
        ...
        if(0 != i)
            idx = v_c[i - 1];
        double p = (double)v_c[i] / (double)size_x;
        if(0.0 == p)
            return DBL_MAX;
        result += p * entropy(t + idx, v_c[i], size_v);
    }
    FREE_POINTERV(v);
    FREE_POINTERV(v_c);
    FREE_POINTERV(t);
...
double info(double* x, int* pos, double* y, int size_x, int size_v) {
    int* v;
    int* v_c;
    double* t;
    double result = 0.0;
...
double gain(double* x, double* y, int size_x, int size_v) {
    double info_d = entropy(y, size_x, size_v);
    double info_split = entropy(x, size_x, size_v) + info_d;
    return (info_d - info(x, y, size_x, size_v)) / info_split;
}
double gain(double* x, int* pos, double* y, int size_x, int size_v) {
    double info_d = entropy(y, pos, size_x, size_v);
    double info_split = entropy(x, pos, size_x, size_v) + info_d;
    return (info_d - info(x, pos, y, size_x, size_v)) / info_split;
}
int gain(double* x, double* y, int size_x, int num_x, int size_v) {
    double max = DBL_MIN;
    int idx_max = -1;
    for(int i = 0; i < num_x; ++i) {
        double tmp = gain(x + i * size_x, y, size_x, size_v);
        if(max < tmp) {
            max = tmp;
            idx_max = i;
        }
...
int gain(double* x, int* pos, double* y, int size_x, int num_x, int size_v) {
    double max = DBL_MIN;
    int idx_max = -1;
    for(int i = 0; i < num_x; ++i) {
        double tmp = gain(x + i * size_x, pos, y, size_x, size_v);
        if(max < tmp) {
            max = tmp;
            idx_max = i;
        }
...
int gain(double* x, int* pos, double* y, int* exist, int size_x, int num_x, int size_v, int size_e) {
    double max = DBL_MIN;
    int idx_max = -1;
    for(int i = 0; i < num_x; ++i) {
        bool b_exist = false;
...
int gain(double* x, int* pos, double* y, int* exist, int size_x, int num_x, int size_v, int size_e, int h) {
    double max = DBL_MIN;
    int idx_max = -1;
    for(int i = 0; i < num_x; ++i) {
        bool b_exist = false;
    ...
    if(-1 == idx_max)
        idx_max = 1000 + (int)y[pos[0]];
    return idx_max;
}
void split(double* x, double* y, int size_x, int* t, int* v_c, int size_v) {
    int* v;
    double result = 0.0;
    NEW_POINTERV(v, int, size_v);
    set_value(0, v, size_v);
    set_value(0, v_c, size_v);
    set_value(0, t, size_x);
    for(int i = 0; i < size_x; ++i) {
        v[(int)x[i]]++;
        v_c[(int)x[i]]++;
    }
...
void split(double* x, int* pos, double* y, int size_x, int* t, int* v_c, int size_v) {
    int* v;
    double result = 0.0;
    NEW_POINTERV(v, int, size_v);
    set_value(0, v, size_v);
...
void set_value(int value, int* vec, int size) {
...
void copy(double** x, double** y, int row_size, int col_size) {
    for(int i = 0; i < row_size; ++i) 
...
double transpose(double* x, double* y, int size_m, int size_n) {
    for(int i = 0; i < size_m; ++i)
        for(int j = 0; j < size_n; ++j)
            y[j * size_m + i] = x[i * size_n + j];
}
double rbf(double x, double delta) {
    return exp(x / (-1 * delta * delta));
}
void rbf(double* x, double* y, double* k, int size, int num_x, double delta) {
    double* z = 0;
    NEW_POINTERV(z, double, num_x * size);
    for(int i = 0; i < num_x; ++i) {
        int step = i * size;
        minos(x, y, z, size);
        multi(z + step, z + step, k + i, 1, size, 1);
    }
    for(int i = 0; i < num_x; ++i)
        k[i] = rbf(k[i], delta);
    FREE_POINTERV(z);
    ...
    for(int i = 0; i < num_x; ++i)
        for(int j = 0; j < num_x; ++j) {
            ...
            for(int k = 0; k < size; ++k) {
                double diff = x[i * size + k] - x[j * size + k];
                sum += diff * diff;
            }
            z[i * num_x + j] = rbf(sum, delta);
        }
}
...
void distance(double* x, double* y, double* z, int size, int num_x) {
    for(int i = 0; i < num_x; ++i)
        z[i] = distance(x + i * size, y, size);
}
void means(double* x, double* y, int size, int num_x) {
    for(int i = 0; i < size; ++i)
        x[i] = (x[i] * num_x + y[i]) / (num_x + 1);
}
void sort_k(double* x, int* y, double* K, int* idx_K, int size, int size_k) {
    for(int i = 0; i < size; ++i) {
...
int max_repeat(int* x, int* y, int size_x, int size_y) {
    for(int i = 0; i < size_x; ++i)
        y[x[i]] += 1;
    int max  = -1;
    int idx_max  = -1;
...
void multi(double* x, double* y, double* z, int size_m, int size_n) {
    for(int i = 0; i < size_m; ++i)
        for(int j = 0; j < size_n; ++j)
            z[i * size_n + j] = x[i * size_n + j] * y[i * size_n + j];
}
void multi(double* x, double* z, int size, int num) {
    for(int i = 0; i < num; ++i)
        for(int j = 0; j < num; ++j)
            for(int k = 0; k < size; ++k)
                z[i * num + j] += x[i * size + k] * x[j * size + k];
}
double sec(double x) {
...
void load_heart_scale(const char* file, double* data, int* label, int size_in) {
    ...
    while(f >> label[i]) {
        for(int k = 0; k < size_in; ++k) {
            f >> idx >> comma >> data[i * size_in + idx - 1];
            if(idx == size_in)
                break;
        }
        i++;
...
void reshape(double* x, double* y, int size, int num) {
    for(int i = 0; i < num; ++i)
        for(int j = 0; j < size; ++j)
            y[j * num + i] = x[i * size + j];
}
